...
const AuthenticationController = {
  register: async (req, res) => {
    try {
      const { firstname, lastname, email, gender, phone,username,password } = req.body;
      ...
      await newUser.save(); // Registering user
  ...
  registerAdmin: async (req, res) => {
    try {
      const { firstname, lastname, email, gender, phone,username,password,role } = req.body;
  ...
  login: async (req, res) => {
    try {
      ...
      const user = await Users.findOne({ email, role: "user" }).populate(
        "folks folksrequested blockedfolks",
        "-password"
      );
      ...
      jwt.verify(
        ...
        async (err, result) => {
          ...
          const user = await Users.findById(result.id)
            .select("-password")
            .populate("folks folksrequested blockedfolks", "-password");
